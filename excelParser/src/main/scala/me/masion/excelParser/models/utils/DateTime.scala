package me.masion.excelParser.models.utils
...
final case class DateTime private (year: Int, // the year
   ...
   * `yyyy-mm-dd`
   */
  def toIsoDateString = s"${(year)}-${(month)}-${(day)}"

  /**
   * `yyyy-mm-ddThh:mm:ss`
   */
  def toIsoDateTimeString = s"${toIsoDateString}T${hour}:${minute}:${second}"

  /**
   * `yyyy-mm-dd hh:mm:ss`
   */
  def toIsoLikeDateTimeString = s"${(hour)}:${(minute)}:${(second)}"
...
object DateTime {
  ...
  val millisTo1970 = 25568 * 86400 * 1000
  ...
  val MinValue = DateTime(1900, 1, 1)
  val MaxValue = DateTime(9999, 12, 31, 23, 59, 59)
  ...
  def apply(year: Int, month: Int, day: Int, hour: Int = 0, minute: Int = 0, second: Int = 0): DateTime = {
    require(1900 <= year && year <= 9999, "year must be >= 1800 and <= 9999")
